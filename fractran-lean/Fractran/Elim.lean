...
/-!
# Static Fraction Elimination (proof side)

The runtime data definitions and runners live in `Fractran.Runtime.Elim`.
This file adds:

- `optTable_size`
- the Nat-keyed wrapper `elimRunNat` (uses `RegMap.facmap` from mathlib)
- the `ElimInvariant` predicate
- all correctness lemmas culminating in `elimRun_correct : Correct elimRunNat`
-/
